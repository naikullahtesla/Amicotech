...
summary: "Learn how to use custom components in markdown files."
tag: "Components"
---

The website will replace some default HTML elements to Once UI components to integrate better in the design and add additional functionality. Furthermore, many common Once UI components are already imported and available for use.
...
Since this website is based on Once UI, you can use all the components provided by Once UI in the `.mdx` files, but you need to import them first in the `src/components/mdx.tsx` file and pass it to the `components` object.
